...
class StartPage(tk.Frame):
        ...
        def __init__(self, parent, controller):
            ...
            label = tk.Label(self, text="        Anasayfa        ", font=self.controller.title_font,fg="#263942")
            label.grid(row=0, sticky="ew")
            button1 = tk.Button(self, text="   Kullanıcı Ekle  ", fg="#ffffff", bg="#263942",command=lambda: self.controller.show_frame("PageOne"))
            button2 = tk.Button(self, text="   Kullanıcı Tanı  ", fg="#ffffff", bg="#263942",command=lambda: self.controller.show_frame("PageTwo"))
            button3 = tk.Button(self, text="Çıkış", fg="#263942", bg="#ffffff", command=self.on_closing)
...
class PageOne(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller = controller
        tk.Label(self, text="İsminiz: ", fg="#263942", font='Helvetica 12 bold').grid(row=0, column=0, pady=10, padx=5)
    ...
    def start_training(self):
        ...
        if self.user_name.get() == "None":
            messagebox.showerror("Hata", "Lütfen isim giriniz.")
            return
        elif self.user_name.get() in names:
            messagebox.showerror("Hata", "Kullanıcı daha önce tanımlanmmış.")
            return
...
class PageTwo(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        global names
        self.controller = controller
        tk.Label(self, text="Kullanıcı seçin: ", fg="#263942", font='Helvetica 12 bold').grid(row=0, column=0, padx=10, pady=10)
        self.buttoncanc = tk.Button(self, text="İptal", command=lambda: controller.show_frame("StartPage"), bg="#ffffff", fg="#263942")
...
class PageThree(tk.Frame):
    ...
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.controller = controller
        self.numimglabel = tk.Label(self, text="Kaydedilen görüntü sayısı = 0", font='Helvetica 12 bold', fg="#263942")
        self.numimglabel.grid(row=0, column=0, columnspan=2, sticky="ew", pady=10)
        self.capturebutton = tk.Button(self, text="Veri kaydet", fg="#ffffff", bg="#263942", command=self.capimg)
        self.trainbutton = tk.Button(self, text="Modeli Eğit", fg="#ffffff", bg="#263942",command=self.trainmodel)
    ...
    def capimg(self):
        self.numimglabel.config(text=str("Kaydedilen görüntüler = 0 "))
        messagebox.showinfo("Talimatlar", "Yüzünün 300 adet fotoğrafının çekilmesi gerekmektedir.")
        x = start_capture(self.controller.active_name)
        self.controller.num_of_images = x
        self.numimglabel.config(text=str("Kaydedilen görüntü sayısı = "+str(x)))

    def trainmodel(self):
        if self.controller.num_of_images < 300:
            messagebox.showerror("Hata", "Yeterli veri yok. En az 300 veri kaydetmen gerekiyor.")
            return
...
class PageFour(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="Yüz Tanımlayıcı", font='Helvetica 16 bold')
        label.grid(row=0,column=0, sticky="ew")
        button1 = tk.Button(self, text="Yüz Tanımlayıcı", command=self.openwebcam, fg="#ffffff", bg="#263942")
        ...
        button4 = tk.Button(self, text="Anasayfaya git", command=lambda: self.controller.show_frame("StartPage"), bg="#ffffff", fg="#263942")
